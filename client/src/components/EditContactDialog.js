...
export default function EditContactDialog(props) {
  const {queryClient, open, onOpenChange, contactId, onContactIdChange} = props;
  const [contact, setContact] = useState(initialContact)
  const [formIsValid, setFormIsValid] = useState(true)
  const [errors, setErrors] = useState(initialErrors)
  const [openSnackbar, setOpenSnackbar] = React.useState(false);

  const { data } = useQuery(["contact"], () => getContact(contactId), {enabled: open});
  ...
  const { mutate, isLoading } = useMutation(editContact, {
    ...
    onSettled: () => {
      queryClient.invalidateQueries('contacts', 'contact');
    }
  });

  React.useEffect(() => {
    if (contactId !== null && data) {
      setContact(data);
    }
  }, [data, contactId]);
  ...
  const handleSave = event => {
    setContact(contact);
    mutate({id: contactId, contact: contact});
  }
